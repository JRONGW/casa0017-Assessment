...
function colorFromValue01(v) {
  ...
  var H = 95;                // yellow-ish green
  var S = 60;                // more vivid
  var L = 90 - num * 55;     // light green → darker yellow-green
...
function updateCash(countryName, val01) {
  var id = idFromName(countryName);
  var el = document.getElementById("stack-" + id);
  if (!el) return;

  el.innerHTML = "";  // remove old images

  var maxNotes = 10;
  var count = Math.round((val01 || 0) * maxNotes);

  for (var i = 0; i < count; i++) {
    var img = document.createElement("img");
    img.src = "./img/money.png";
    img.className = "cash-note";
    /* horizontal offset so notes look like a stack */
    img.style.left = (i * 22) + "px";
    el.appendChild(img);
  }
}
